...
public struct StoreManagerOnboardingView: View {
    let user: User
    ...
    public init(user: User, onComplete: @escaping (User) -> Void, onLogout: @escaping () -> Void) {
        self.user = user
...
struct ProfileCompletionScreen: View {
    ...
    init(user: User, path: Binding<NavigationPath>, onComplete: @escaping (User) -> Void) {
        self.user = user
        self._path = path
        self.onComplete = onComplete
